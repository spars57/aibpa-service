...
class MessageRepository {
  ...
  public async create(message: Omit<Message, 'id' | 'uuid' | 'timestamp'>): Promise<Message> {
    const newMessage = await this.prisma.message.create({
      data: {
        ...
        created_at: this.getCurrentDate(), // Define o timestamp atual para a mensagem
      },
    })
    return newMessage
  }
  ...
  public async update(id: number, data: Partial<Message>): Promise<Message> {
    ...
    const updatedMessage = await this.prisma.message.update({
      ...
      data: {
        ...data,
        created_at: this.getCurrentDate(), // Atualiza o timestamp da mensagem
      },
    })
  ...
  public async getMessagesWithPagination(chatId: number, page: number = 1, limit: number = 10): Promise<Message[]> {
